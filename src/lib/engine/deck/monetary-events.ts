...
export const MONETARY_EVENTS: EventCard[] = [
  {
    ...
    descriptionAr: 'مع اتساع الفجوة التضخمية، عجزت فروع المصرفين التجاري والعقاري عن تلبية سحوبات الموظفين والمتقاعدين، وتكدست الطوابير لمئات الأمتار في دمشق وحلب وسط شائعات عن نفاد أوراق فئة الـ 50 SP.',
    targetGovernorateId: 'damascus',
    triggerCondition: (state) => state.macro.treasurySYP < 40_000_000_000 || state.macro.annualInflationPct > 20,
    options: [
      {
        id: 'opt_emergency_print',
        labelAr: 'طباعة إصدار نقدي إسعافي فوري دون تغطية',
        descriptionAr: 'ضخ كتلة نقدية سريعة لملء الصرافات ودفع الرواتب فوراً على حساب تسارع التضخم وانهيار القوة الشرائية لSP.',
        costUSD: 0,
        ...
        effectCorruption: 4,
        effectCompetence: -5,
        customEffectAr: 'توفير سيولة فورية (+12B SP) مع تراجع ثقة الأسواق وتصاعد التضخم',
        governorateEffects: [
      ...
      },
      {
        ...
        descriptionAr: 'تقييد السحوبات النقدية بـ 3,000 SP يومياً وإلزام التجار بالدفع الرقمي لتفادي نفاد النقد.',
        costUSD: 0,
        costSYP: 0,
        costPC: 15,
        effectTrust: -18,
  ...
  },
  {
    ...
    triggerCondition: (state) => state.macro.parallelRateSYP > 135 || state.macro.systemicCorruption > 30,
    options: [
      {
        ...
        effectCorruption: 6,
        effectCompetence: -2,
        customEffectAr: 'مصادرات بـ 8B SP مع انقطاع الحوالات عن الأسر (+18 احتقان شعبي)',
        governorateEffects: [
  ...
  },
  {
    ...
    targetGovernorateId: 'damascus',
    triggerCondition: (state) => (state.macro.taxCompliancePct ?? 60) < 65 || state.macro.treasurySYP < 50_000_000_000,
    options: [
      {
      ...
      },
      {
        ...
        effectCorruption: -5,
        effectCompetence: 12,
        customEffectAr: 'امتصاص فائض السيولة بـSP وتعزيز ثقة المدخرين بالمؤسسة الرسمية (+10 ثقة)',
        governorateEffects: [
    ...
    options: [
      {
        ...
        costUSD: 0,
        costSYP: -6_500_000_000,
        costPC: 12,
        effectTrust: -10,
        effectRRI: 14,
        effectCorruption: -10,
        effectCompetence: 12,
        customEffectAr: 'تحصيل 6.50B SP إيرادات ضريبية مع نقص سلع مؤقت في الأسواق (+14 احتقان)',
        governorateEffects: [
    ...
    options: [
      {
        ...
        costUSD: 0,
        costSYP: -9_000_000_000,
        costPC: 18,
        effectTrust: 14,
        effectRRI: -8,
        effectCorruption: -16,
        effectCompetence: 10,
        customEffectAr: 'مصادرة 9B SP واستعادة هيبة الدولة ومكافحة الفساد الصارم (+14 ثقة)',
        governorateEffects: [
  ...
  },
  {
    ...
    triggerCondition: (state) => state.macro.civicTrust < 60 || state.macro.parallelRateSYP > 130,
    options: [
      {
        ...
        effectCorruption: 2,
        effectCompetence: 6,
        customEffectAr: 'تهدئة الشارع الحمصي وحماية الأسر الضعيفة مع تعميق العجز العام (-12B SP)',
        governorateEffects: [
